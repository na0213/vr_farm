<x-admin-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('admin.backend.products.update', ['id' => $product->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                @if ($errors->any())
                ...
                <div class="-m-2">
                    <div class="p-2 w-4/5 mx-auto">
                        ...
                        <input type="text" id="product_link" name="product_link" value="{{ $product->product_link }}" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-yellow-500 focus:bg-white focus:ring-2 focus:ring-yellow-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-container m-4">
                    <div class="image-input">
                        <label for="product_image" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">画像<span class="text-red-600">（3MB以下）</span></label>
                        ...
                        <div class="image-preview mt-2" id="preview_image">
                            <img src="{{ $product->product_image }}" alt="Image preview" style="width: 200px; height: auto;">
                        </div>
                    </div>
                </div>
            ...
            <div class="p-2 w-full flex justify-around mt-4">
                <form action="{{ route('admin.backend.products.destroy', ['id' => $product->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
